...
public final class GeocellUtils {
     ...
     */
    public static boolean containsPoint(String cell, LatLng point) {
     ...
     */
    public static double pointDistance(String cell, LatLng point) {
        BoundingBox bbox = computeBox(cell);

        boolean betweenWE = bbox.getWest() <= point.longitude && point.longitude <= bbox.getEast();
        boolean betweenNS = bbox.getSouth() <= point.latitude && point.latitude <= bbox.getNorth();

        if(betweenWE) {
            if(betweenNS) {
                // Inside the geocell.
                return Math.min(
                        Math.min(distance(point, new LatLng(bbox.getSouth(), point.longitude)),distance(point, new LatLng(bbox.getNorth(), point.longitude))),
                        Math.min(distance(point, new LatLng(point.latitude, bbox.getEast())),distance(point, new LatLng(point.latitude, bbox.getWest()))));
            } else {
                return Math.min(distance(point, new LatLng(bbox.getSouth(), point.longitude)),distance(point, new LatLng(bbox.getNorth(), point.longitude)));
            }
        } else {
            if(betweenNS) {
                return Math.min(distance(point, new LatLng(point.latitude, bbox.getEast())),distance(point, new LatLng(point.latitude, bbox.getWest())));
            } else {
                // TODO(romannurik): optimize
                return Math.min(Math.min(distance(point, new LatLng(bbox.getSouth(), bbox.getEast())),distance(point, new LatLng(bbox.getNorth(), bbox.getEast()))),
                        Math.min(distance(point, new LatLng(bbox.getSouth(), bbox.getWest())),distance(point, new LatLng(bbox.getNorth(), bbox.getWest()))));
            }
        }
    }
    /**
     ...
     */
    public static String compute(LatLng point, int resolution) {
        ...
        while(cell.length() < resolution) {
            ...
            int x = Math.min((int)(GEOCELL_GRID_SIZE * (point.longitude - west) / (east - west)),
                    GEOCELL_GRID_SIZE - 1);
            int y = Math.min((int)(GEOCELL_GRID_SIZE * (point.latitude - south) / (north - south)),
                    GEOCELL_GRID_SIZE - 1);
	...
	public static double distance(LatLng p1, LatLng p2) {
		...
		double p1lon = Math.toRadians(p1.longitude);
		double p2lat = Math.toRadians(p2.latitude);
		double p2lon = Math.toRadians(p2.longitude);
		return RADIUS
