...
public class GetUserActivityService extends BaseService {
    ...
    public static void startForRequest(Context context, int requestId, long userId) {
        ApiUserIdRequest apiUserIdRequest = new ApiUserIdRequest(requestId, userId);
        addRequest(context, GetUserActivityService.class, apiUserIdRequest, mRequestList);
    }
    ...
    protected void requestProcess(final ApiBaseRequest apiBaseRequest) {
        ...
        Call<List<UserActivity>> call = mApiServer.getUserActivity(apiUserIdRequest.getUserId());
        call.enqueue(new Callback<List<UserActivity>>() {
            ...
            public void onResponse(Call<List<UserActivity>> call, Response<List<UserActivity>> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    result = new ApiGetUserActivityResult(response.body());
                } else {
                ...
                notifyResultListener(apiBaseRequest.getId(), apiUserIdRequest, result, response, null);
            }

            @Override
            public void onFailure(Call<List<UserActivity>> call, Throwable retrofitError) {
