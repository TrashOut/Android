...
public class CreateUserService extends BaseService {
    ...
    public static void startForRequest(Context context, int requestId, User user) {
        ApiUserRequest apiUserRequest = new ApiUserRequest(requestId, user);
        addRequest(context, CreateUserService.class, apiUserRequest, mRequestList);
    }
    ...
    protected void requestProcess(final ApiBaseRequest apiBaseRequest) {
        ...
        ApiUserRequest apiUserRequest = (ApiUserRequest) apiBaseRequest;


        Call<User> call = mApiServer.createUser(apiUserRequest.getUser());
